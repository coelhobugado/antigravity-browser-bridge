...
<p className="text-[#888] text-sm">July 24, 2026</p>

### Fixes

- Aligned the extension and native host on the same protocol version and restored the connection handshake.
- Made installation configure `~/.gemini/config/mcp_config.json` automatically while preserving existing servers and rejecting malformed JSON.
- Removed disconnected experimental modules that simulated IPC, policies, and domain adapters without executing the real flow.
- Hardened the popup and content script with cryptographic nonces, safe rendering, session authorization, and improved element resolution.
- Prevented `doctor` from hanging while querying Chrome on Windows and made the recording test respect its optional FFmpeg dependency.

### Improvements

- Updated the extension to `1.2.0` with tab authorization controls, connection status, and recent activity.
- Updated the Portuguese and English guides, CLI documentation, and core skill.
...
### WorkService and stability

- Added the typed `WorkService` for the `antigravity-work` MCP profile with state machine, deadlines, idempotency, cancellation, journal, checkpoints, resume, redacted export, and real status.
- Separated the MCP adapter from business logic and added an on-demand asynchronous runtime with per-instance locking.
- Updated contracts, documentation, extension validation, evaluations, and release artifacts.
- Updated the extension to `1.1.2`.
...
- Added **Portuguese and English setup guides** for installation, MCP configuration, and troubleshooting
