...
agent-browser mcp --tools antigravity-work
```

Configure the MCP client to launch `agent-browser` with `["mcp"]`. The server defaults to MCP protocol 2025-11-25 and accepts older supported client protocol versions during initialization. The default tools profile is `core`, which keeps MCP context small for everyday browser automation. Use `--tools all` for the full typed CLI parity surface, or combine profiles with commas, such as `--tools core,network,react`. Profiles are `core`, `network`, `state`, `debug`, `tabs`, `react`, `mobile`, `antigravity-work`, and `all`; the `debug` profile includes accessibility audits, plugin registry, and command.run tools. The `antigravity-work` profile operates only personal Chrome tabs explicitly authorized through the connector. Each tool accepts typed arguments plus `extraArgs` for advanced CLI flags and exact CLI parity. The common `allowedDomains` array maps to `--allowed-domains` and activates the same WebRTC containment and launch-mode restrictions. Tool discovery is paginated and includes read-only/open-world annotations so modern MCP clients can load the large typed surface incrementally. Use the tool `session` argument or `AGENT_BROWSER_SESSION` to isolate browser sessions.

For the personal Chrome connector, run `agent-browser antigravity install`, which registers the native host and adds the `antigravity-work` MCP server to `~/.gemini/config/mcp_config.json` without replacing other servers. Load the unpacked extension, open its toolbar popup, and authorize each tab that the agent may use. Run `agent-browser antigravity doctor` to validate the host. The bundled extension has a stable ID derived from its manifest key, so normal installation never asks the user or agent to copy an ID. `AGENT_BROWSER_EXTENSION_ID` is only an advanced override for separately repackaged development builds. The `antigravity-work` tools are backed by a typed WorkService with explicit state transitions, deadlines, idempotency keys, cooperative cancellation, checkpoints, resume, redacted export, and journal inspection/deletion. Pass the returned `workId` to every work operation and use `idempotencyKey` for retries. Never treat a `not_implemented` response from a work tool as success.
